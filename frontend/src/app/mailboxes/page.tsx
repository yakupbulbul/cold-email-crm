...
import { Plus, Globe, Mail, Edit2, Trash2 } from 'lucide-react';
      ...
      {activeTab === 'mailboxes' ? (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {[
                { email: 'john@example.com', status: 'Active', limit: '45/50' },
                { email: 'sarah@example.com', status: 'Warming Up', limit: '12/15' },
              ].map((mb, i) => (
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium">SMTP/IMAP Active • mail.example.com</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex justify-between items-center text-xs font-semibold text-slate-600">
                        <span>{mb.limit}</span>
                        <span>{mb.status === 'Active' ? '90%' : '80%'}</span>
                      </div>
                      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden shadow-inner">
                        <div className={`h-full rounded-full ${mb.status === 'Active' ? 'bg-gradient-to-r from-green-400 to-green-500' : 'bg-gradient-to-r from-yellow-400 to-amber-500'}`} style={{ width: mb.status === 'Active' ? '90%' : '80%' }}></div>
                      </div>
                    </div>
                  </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col items-center justify-center p-16">
           ...
           <h3 className="text-xl font-bold text-slate-800 mb-2">No Domains Found</h3>
           <p className="text-sm text-slate-500 mb-6 max-w-sm text-center">Add your first domain to start configuring mailboxes and sending emails. You will need DNS access.</p>
