...
export default function JobQueueDashboard() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "dead_letter": return <span className="px-2 py-1 bg-[#3d2e22] text-white font-bold rounded-lg text-xs flex items-center gap-1 w-fit"><AlertTriangle size={12}/> Dead Letter</span>;
            case "queued": return <span className="px-2 py-1 bg-amber-100 text-amber-700 font-bold rounded-lg text-xs">Queued</span>;
            case "running": return <span className="px-2 py-1 bg-[#d4e4d3] text-[var(--primary)] font-bold rounded-lg text-xs animate-pulse">Running</span>;
            default: return <span className="px-2 py-1 bg-[var(--surface-muted)] text-[var(--foreground)] font-bold rounded-lg text-xs">{status}</span>;
        }
    };
        ...
        <div className="space-y-6 animate-fade-in relative min-h-screen pb-12">
            ...
            <div className="grid grid-cols-4 gap-4 mt-6">
                <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)]">
                    <div className="text-sm font-bold text-[var(--muted-foreground)]">Queued</div>
                    <div className="text-2xl font-black text-amber-600">{stats.queued || 0}</div>
                </div>
                <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)]">
                    <div className="text-sm font-bold text-[var(--muted-foreground)]">Running</div>
                    <div className="text-2xl font-black text-[var(--primary)]">{stats.running || 0}</div>
                </div>
                <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)]">
                    <div className="text-sm font-bold text-[var(--muted-foreground)]">Failed / Dead Letter</div>
            ...
            <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] shadow-sm p-6 mt-8">
                <div className="flex gap-2 mb-6 border-b border-[var(--surface-muted)] pb-4">
                    {["all", "queued", "running", "failed", "dead_letter"].map(f => (
                        <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 font-bold rounded-lg text-sm transition-colors ${filter === f ? "bg-[var(--sidebar)] text-white" : "bg-[var(--surface-muted)] text-[var(--muted-foreground)] hover:bg-[var(--surface-muted)]"}`}>
                ...
                {loading ? <Spinner size="lg" /> : (
                    <Table columns={["Job ID", "Task Target", "Status", "Retries", "Duration", "Controls"]}>
                        {jobs.map(job => (
                            <TableRow key={job.id}>
                                <TableCell className="font-mono text-xs font-bold text-[var(--muted-foreground)]">{job.job_id?.substring(0,8)}</TableCell>
                                <TableCell className="font-bold text-[var(--foreground)]">{job.job_type}</TableCell>
                                <TableCell>{getStatusBadge(job.status)}</TableCell>
                                <TableCell className="font-bold text-[var(--muted-foreground)]">{job.retry_count}</TableCell>
                                <TableCell className="text-[var(--muted-foreground)] font-medium">
                                ...
                                <TableCell>
                                    <div className="flex gap-2">
                                        {(job.status === "failed" || job.status === "dead_letter") && (
                                            <button onClick={() => handleRetry(job.job_id)} className="p-2 bg-[#d4e4d3] text-[var(--primary)] hover:bg-[#d4e4d3] rounded-lg transition-colors" title="Retry Job">
                                                <RefreshCw size={16} />
                                            </button>
                                        )}
                                        {job.status === "queued" && (
