...
| 34 | `/api/v1/ops/readiness` reports the expected structured state for the chosen mode (`degraded` in lean mode is acceptable if workers are intentionally blocked) | ☐ | |
...
| 38 | DKIM/SPF/DMARC records validated in DNS | ☐ | |
