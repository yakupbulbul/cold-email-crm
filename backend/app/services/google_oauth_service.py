...
class GoogleWorkspaceOAuthService:
    ...
    def disconnect(self, mailbox: Mailbox) -> Mailbox:
        ...
        mailbox.oauth_last_checked_at = datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    def get_valid_access_token(self, mailbox: Mailbox) -> str:
        ...
        if token.token_expiry and token.token_expiry > datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=2):
    ...
    def _persist_token(self, mailbox: Mailbox, payload: dict, *, preserve_refresh_token: bool = False) -> MailboxOAuthToken:
        ...
        token.token_expiry = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(seconds=expires_in)
        ...
        token.last_refreshed_at = datetime.now(timezone.utc).replace(tzinfo=None)
        token.last_error = None
